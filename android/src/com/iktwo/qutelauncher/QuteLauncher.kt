	...
	companion object
	{
		...
		fun applications():Array<Application>
		{
			...
			for(i in availableActivities.indices)
			{
				applications.add(
					Application(
						availableActivities[i].loadLabel(mPm).toString(),
			...
			{
				if(applications[i].name.isNotEmpty() && (Character.isSpaceChar(
						applications[i].name[0]
					) || Character.isWhitespace(applications[i].name[0]))
				)
				{
					val charToReplace = applications[i].name[0]
					applications[i].name =
						applications[i].name.replace(charToReplace, ' ')
							.trim {it <= ' '}
		...
		fun getApplicationIcon(packageName:String):ByteArray
		{
                        ...
                        if(mPm != null)
                        {
                            ...
                            if(app != null)
                            {
                                ...
                                if(intent != null)
                                {
                                    val resolveInfo = mPm?.resolveActivity(intent, 0)
                                    if(resources != null && resolveInfo != null)
                                    {
                                        icon = resources.getDrawableForDensity(
                                                resolveInfo.activityInfo.iconResource, mIconDpi
                                        )
                                    }
                                }
                            }
                        ...
                        //	icon = null
                        //}
			if(icon == null) icon = defaultApplicationIcon
			...
			else
			{
				if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.O)
				{
					...
					{
						val arrayOfDrawables =
							arrayOf(icon.background, icon.foreground)
					...
					{
						bitmap =
							(defaultApplicationIcon as BitmapDrawable).bitmap
					}
				}
				else
				{
		...
		val defaultApplicationIcon:Drawable
			get() = instance!!.getResources()
				.getDrawable(android.R.mipmap.sym_def_app_icon)
		@JvmStatic
		fun getApplicationLabel(packageName:String):String
		{
			try
			{
                                ...
                                if(intent != null)
                                {
                                    ...
                                    val label = resolveInfo?.loadLabel(mPm)
